...
The charts below show the relative wealth of metropolitan areas to the national average from 1969 to 2022. The data comes from the BEA (Bureau of Economic Analysis) and the data only goes as far back as 1969.

Notable trends:

1) Austin-Round Rock-Georgetown, TX actually used to be poorer than average until quite recently.

2) Detroit-Warren-Dearborn, MI had a large relative decline from 1969 to 2022. Actually, nearly all cities in Michigan are declining relative to the national average. The same applies for many nearby states, which is a reason why this area is often called the "Rust Belt".
...
Relative = bind_rows(apply(Table[3:56], 1, function(x) x/Table[1,3:56]))
Relative = cbind(Table[2], Relative)
Relative = pivot_longer(Relative, cols = 2:55, names_to = "Year", values_to = "Relative")
...
make_plot = function(x){
  ...
  name = paste0("Relative Personal Income per Capita (US = 100%) 1969-2022 for \n", x[[1]][1])
...
make_plot_state = function(x){
  ...
  minimum = min(0.8, min(x[[3]]))
  maximum = max(1.2, max(x[[3]]))
  name = paste0("Relative Personal Income per Capita (US = 100%) 1969-2022 for cities in \n", x[[4]][1])
...
make_plot_city = function(x){
  minimum = min(0.8, min(x[[3]]))
  maximum = max(1.2, max(x[[3]]))
  name = paste0("Relative Personal Income per Capita (US = 100%) 1969-2022")
  ggplot(x, aes(x = Year, y = Relative, color = GeoName)) +
    scale_y_continuous(labels = scales::label_percent()) +
    geom_point() +
    geom_line() +
...
```{r}
Table <- read_excel("Data/Table (5).xlsx",skip = 5)
temp = stringr::str_split(Table[[2]], " \\(")
Table[[2]] = sapply(temp, function(x) x[1])
Relative = bind_rows(apply(Table[3:56], 1, function(x) x/Table[1,3:56]))
Relative = cbind(Table[2], Relative)
...
make_plot = function(x){
  minimum = min(0.8, min(x[[3]]))
  maximum = max(1.2, max(x[[3]]))
  name = paste0("Relative Average Wages and Salaries (US = 100%) 1969-2022 for \n", x[[1]][1])
  ggplot(x, aes(x = Year, y = Relative)) +
    geom_point() +
    geom_line() +
    scale_y_continuous(labels = scales::label_percent()) +
...
make_plot_state = function(x){
  ...
  minimum = min(0.8, min(x[[3]]))
  maximum = max(1.2, max(x[[3]]))
  name = paste0("Relative Average Wages and Salaries (US = 100%)  1969-2022 for cities in \n", x[[4]][1])
  ggplot(x, aes(x = Year, y = Relative, color = GeoName)) +
    geom_point() +
    geom_line() +
    theme_bw() + 
...
make_plot_city = function(x){
  minimum = min(0.8, min(x[[3]]))
  maximum = max(1.2, max(x[[3]]))
  name = paste0("Relative Average Wages and Salaries (US = 100%) 1969-2022")
  ggplot(x, aes(x = Year, y = Relative, color = GeoName)) +
    geom_point() +
    geom_line() +
    theme_bw() + 
